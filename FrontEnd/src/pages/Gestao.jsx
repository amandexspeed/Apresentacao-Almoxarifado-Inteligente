...
function Gestao() {
  ...
  async function changeEmailStatus(status,id) {
    try {
      await axios.patch(`https://localhost:8014/api/Produtos/${id}`, {
        emailStatus: status,
      });
    } catch (error) {
  ...
  async function doBench(idproduto, nome) {
    try {
      let response = await axios.get(
        `https://localhost:8014/api/Benchmarking/compare?descricaoProduto=${nome}&idProd=${idproduto}`
      );
  ...
  async function changeStatusPrice(status, price, idProduto) {
    try {
      let response = await axios.patch(
        `https://localhost:8014/api/Produtos/${idProduto}`,
        {
          preco: price,
          status: status,
        }
  ...
  async function getBench(idProduto) {
    try {
      let response = await axios.get(
        `https://localhost:8014/api/BenchmarkingItems/${idProduto}`
      );
      let mensagem = "uhu";
      if(response){
  ...
  const getData = useCallback(async () => {
    const response = await axios.get("https://localhost:8014/api/Produtos");
    setList(response.data);
  }, [list]);

  const createNewProduct = useCallback(
    async ({ name, price, estoque, estoqueMin }) => {
      await axios.post("https://localhost:8014/api/Produtos", {
        descricao: name,
        preco: price,
